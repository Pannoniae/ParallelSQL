...
sbatch --job-name="$1" -D /users/40536446/ParallelSQL --output="$1".out --mail-user="40536446@napier.ac.uk" --mail-type=ALL --exclusive --time=90 --ntasks=$total_nodes --nodes=$total_nodes --nodelist node[02-"$(printf "%02d" $end_node)"] --cpus-per-task=$cpus_per_task ./multirunq.sh "$1"